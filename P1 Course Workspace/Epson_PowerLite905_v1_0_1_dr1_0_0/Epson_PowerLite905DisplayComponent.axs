...
(*********************************************************************)
MODULE_NAME = 'Epson_PowerLite905DisplayComponent' (dev vdvDev[], dev dvTP, INTEGER nDevice, INTEGER nPages[])
(***********************************************************)
(* System Type : NetLinx                                   *)
...
#include 'Epson_PowerLite905ComponentInclude.axi'
...
DEFINE_FUNCTION OnZoneChange()
{
	...
	{
		send_command dvTP, "'^TXT-',itoa(TXT_ASPECT_RATIO_SELECT[nCount]),',0,',AspectProps[nCurrentZone][nCount].sLabel"
...
DEFINE_FUNCTION parseProperties(CHAR props[], INTEGER nZone)
{
    ...
    WHILE(LENGTH_STRING(props) > 0)
    {
		...
		if (index > 0 && index <= MAX_ASPECTS)
		{
			...
			send_command dvTP, "'^TXT-',itoa(TXT_ASPECT_RATIO_SELECT[AspectProps[nZone][index].nIndex]),',0,',AspectProps[nZone][index].sLabel"
		}
    }
}
//*********************************************************************
			...
			CASE 'ASPECTRATIOSELECT' :
			{
				...
				IF (index > 0 && index <= MAX_ASPECTS)
				{
					...
					if (nZone == nCurrentZone)
					{
						...
						if (bFound == TRUE)
							send_command dvTP, "'^TXT-',itoa(TXT_ASPECT_RATIO),',0,',AspectProps[nZone][nParameter].sLabel"
					}
				}
			}
