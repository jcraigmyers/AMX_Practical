...
VOLATILE CHAR LocalPort
VOLATILE CHAR IPAddress		//String
VOLATILE CHAR IPPort		//Long
...
define_function	fnInitializeLightArray()
{
    ...
    instancedScenes[2].instancedLights[1].lightIntensity	=	0
    instancedScenes[2].instancedLights[1].fadeTime		=	0
    instancedScenes[2].instancedLights[2].lightIntensity	=	0
    instancedScenes[2].instancedLights[2].fadeTime		=	0
    instancedScenes[2].instancedLights[3].lightIntensity	=	0
    instancedScenes[2].instancedLights[3].fadeTime		=	0
    instancedScenes[2].instancedLights[4].lightIntensity	=	0
...
DEFINE_FUNCTION fnReadFromFile()
{
    ...
    *)
    ...
    IF (localport == '1:4:6' && IPAddress == '147.153.59.9' && IPPort == '24' && Protocol == 'tcp')
	{
...
define_module 'VirtualKeypad_dr1_0_0' VKP(VIRTUALPAD,dvVIRTUALKEYPAD)
...
button_event[dvTP_LIGHT,0] 
{
	...
	switch(button.input.channel)
	{
	    ...
	    *)
		...
		//Handles Feedback
		send_command dvTP_LIGHT,"'^TXT-10,0,',instancedScenes[GET_LAST(nSceneButtons)].sceneName";
		...
		ELSE
		{
		    send_command dvLIGHTS,"'RAISEDIM,[1:',ITOA(GET_LAST(nRaiseDimButtons)),']',$0d"; //Translates buttons to zones 1-4 via array index
		   // send_command dvLights,"'FADEDIM,',ITOA(nTensity),',0,0,[1:',ITOA(GET_LAST(nLowerDimButtons)),']',$0d"; //Use direct set for lights. Simplifies everything.
		}
	    }
	}
	...
	switch(button.input.channel)
	{
	    ...
	    case 41:
	    {
		send_command dvLIGHTS,"'STOPDIM,[1:',ITOA(GET_LAST(nRaiseDimButtons)),']',$0d";
	    }
	    case 36:
	    case 38:
	    case 40:
	    case 42:
	    {
		send_command dvLIGHTS,"'STOPDIM,[1:',ITOA(GET_LAST(nLowerDimButtons)),']',$0d";
	    }
	}
    }
}
